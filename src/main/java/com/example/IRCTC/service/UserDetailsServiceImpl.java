...
public class UserDetailsServiceImpl implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		Optional<User> userOptional = userRepository.findByUserName(username);// we took username from DB.

		if (userOptional.isPresent()) {
			User user = userOptional.get(); // Extract User from Optional
			return org.springframework.security.core.userdetails.User.builder()
					.username(user.getUserName()) //Sets the username.
					.password(user.getPassword())//Sets the hashed password (stored in DB).hashed password (DB से लिया गया)
